...
class miRvestigator:
    ...
    def __init__(self, pssms, seqs3pUTR, seedModel=[6,7,8], minor=True,
        ...
        if use_multiprocessing:
            ...
            args = zip(pssms, [wobble] * len(pssms))
            results = pool.map(self.detect_pssm, args)
            for item in results:
                outMe.extend(item)
        else:
            for pssm in pssms:
                res = self.detect_pssm((pssm, wobble))
                outMe.extend(res)
    ...
    def detect_pssm(self, arg):
        pssm, wobble = arg
        result = []
        print('\n' + pssm.getName())
        print('Building HMM model for '+str(pssm.getConsensusMotif())+'...')
        #miRNAScores[pssm.getName()] = ['NA','NA']  ## WW: actually we don't use it
        # Then setup the HMM
        ## States ##
        ...
        if hits_8mer<=1:
            ...
            result.append(pssm.getName()+','+'_'.join(vitMirnas)+',8mer')
        ...
        if hits_8mer>1:
            ...
            if hits_7mer_m8<=1:
                ...
                result.append(pssm.getName()+','+'_'.join(vitMirnas)+',7mer_m8')
            ...
            if hits_7mer_a1<=1:
                ...
                result.append(pssm.getName()+','+'_'.join(vitMirnas)+',7mer_a1')
        if hits_8mer>1 and hits_7mer_m8>1 and hits_7mer_a1>1:
            print('No match!')
            result.append(pssm.getName()+',NA,NA')
        return result
...
if __name__ == '__main__':
    ...
    parser.add_argument('seqs', help='seqs.txt file')
    parser.add_argument('pssms', help='pssms file')
    args = parser.parse_args()

    if not os.path.exists(args.seqs):
        raise Exception('seqs.txt file does not exist')
    if not os.path.exists(args.pssms):
        raise Exception('pssms.json file does not exist')

    with open(args.seqs) as infile:
        seqs = infile.readlines()

    pssms = []
    with open(args.pssms) as infile:
        pssms_json = json.load(infile)
        for pssm_json in pssms_json:
            pssms.append(pssm.pssm(biclusterName=pssm_json['name'],
                                   nsites=pssm_json['nsites'],
    ...
    m2m = miRvestigator(pssms, seqs, seedModel=[6,7,8], minor=True, p5=True,
                        ...
                        use_multiprocessing=True)
    with open('m2m_standalone.pkl', 'wb') as outfile:
